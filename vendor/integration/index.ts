...
export default ({ config: _themeConfig = 'src/config.yaml' } = {}): AstroIntegration => {
  let cfg: AstroConfig;
  // `config.base` on the `astro:config:done` snapshot is the value Astro had
  // before this integration widened it, so the deployment sub-path is kept here
  // and used when writing robots.txt.
  let siteBase = '/';
  return {
      ...
      'astro:build:done': async ({ dir, logger }) => {
        ...
        try {
          ...
          if (hasIntegration && sitemapExists) {
            ...
            // The deployment sub-path needs a trailing slash before it is used
            // as a URL base, otherwise `new URL()` treats the last segment as a
            // file name and drops it.
            const basePath = siteBase.endsWith('/') ? siteBase : `${siteBase}/`;
