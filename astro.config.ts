...
export default defineConfig({
  ...
  // Locale routing for the machine pages. The source landing pages shipped a
  // seven-language dictionary and switched language client-side with `?lang=`;
  // here every locale becomes a real, prerendered URL instead, so each
  // translation is crawlable and gets its own hreflang entry.
  // `site` and `base` are injected by the astrowind integration from
  // src/config.yaml, so they are not repeated here.
  i18n: {
    locales: ['en', 'es', 'pt', 'fr', 'ar', 'ru', 'zh'],
    defaultLocale: 'en',
    routing: {
  ...
  integrations: [
    // The i18n option makes @astrojs/sitemap emit the `xhtml:link
    // rel="alternate" hreflang="…"` entries for every locale we prerender.
    sitemap({
      i18n: {
        defaultLocale: 'en',
        locales: { en: 'en', es: 'es', pt: 'pt', fr: 'fr', ar: 'ar', ru: 'ru', zh: 'zh' },
      },
    }),
    mdx(),
    icon({
